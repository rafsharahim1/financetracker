...
# Set light pink and purple theme
...
class Application(ctk.CTk):
    def __init__(self):
        ...
        self.income_frame, self.income_tree = self.create_transaction_frame("Income", self.add_income, row=1,
                                                                            transactions=self.income_transactions)
        self.expense_frame, self.expense_tree = self.create_transaction_frame("Expense", self.add_expense, row=2,
    ...
    def create_transaction_frame(self, title, button_command, row, transactions):
        ...
        ctk.CTkLabel(frame, text=f"Add {title}", font=ctk.CTkFont(size=20)).grid(row=0, column=0, columnspan=2)
        ...
        ctk.CTkLabel(frame, text="Date").grid(row=3, column=0)
        date_entry = DateEntry(frame)
        date_entry.grid(row=3, column=1)

        ctk.CTkButton(frame, text=f"Add {title}", command=lambda: button_command(category_entry, amount_entry, date_entry)).grid(
            row=4, column=0, columnspan=2)

        quick_buttons_frame = ctk.CTkFrame(frame)
        quick_buttons_frame.grid(row=5, column=0, columnspan=2, pady=10)

        for i, cat in enumerate(["Utility", "School", "Healthcare", "Groceries", "Entertainment", "Travel"]):
            ctk.CTkButton(quick_buttons_frame, text=cat, command=lambda c=cat: self.set_category(category_entry, c)).grid(row=i//3, column=i%3, padx=5, pady=5)
        ...
        tree.grid(row=6, column=0, columnspan=2, padx=20, pady=20)

        return frame, tree

    def set_category(self, category_entry, category):
    ...
    def add_income(self, category_entry, amount_entry, date_entry):
        amount = self.get_amount_from_entry(amount_entry)
        category = category_entry.get()
        date = date_entry.get()
        if amount is not None:
            self.income += amount
            self.income_transactions.append((category, amount, date))
            self.update_table(self.income_tree, self.income_transactions)
            category_entry.delete(0, 'end')
            amount_entry.delete(0, 'end')
            date_entry.set_date(datetime.date.today())

    def add_expense(self, category_entry, amount_entry, date_entry):
        amount = self.get_amount_from_entry(amount_entry)
        category = category_entry.get()
        date = date_entry.get()
        if amount is not None:
            self.expense += amount
            self.expense_transactions.append((category, amount, date))
